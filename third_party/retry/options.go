...
// Factor set factor of the delay between retries
// default is 2
func Factor(factor uint) Option {
	return func(c *Config) {
		c.factor = factor
	}
}
...
// ExponentDelay is a DelayType which picks a exponent delay
func ExponentDelay(_ uint, _ error, config *Config) time.Duration {
	if config.delay == 0 {
		config.delay = DefaultDelay
	}
	if config.factor == 0 {
		config.factor = DefaultFactor
	}
	...
	if dur > float64(config.maxDelay) {
