...
func (service *SubnetService) createOrUpdateSubnet(obj client.Object, nsxSubnet *model.VpcSubnet, vpcInfo *common.VPCResourceInfo) (*model.VpcSubnet, error) {
	...
	if err != nil {
		log.Error(err, "Failed to create or update subnet", "ID", *nsxSubnet.Id)
		return nil, err
	}
...
func (service *SubnetService) DeleteSubnet(nsxSubnet model.VpcSubnet) error {
	nsxOrgID, nsxProjectID, nsxVPCID, nsxSubnetID := nsxutil.ParseVPCPath(*nsxSubnet.Path)
	nsxSubnet.MarkedForDelete = &MarkedForDelete
	err := service.NSXClient.SubnetsClient.Delete(nsxOrgID, nsxProjectID, nsxVPCID, nsxSubnetID)
	err = nsxutil.TransNSXApiError(err)
	if err != nil {
