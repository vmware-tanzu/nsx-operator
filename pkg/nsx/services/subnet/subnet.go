...
func (service *SubnetService) CreateOrUpdateSubnet(obj client.Object, vpcInfo common.VPCResourceInfo, tags []model.Tag) (subnet *model.VpcSubnet, err error) {
	...
	if subnet, ok := obj.(*v1alpha1.Subnet); ok {
		...
		if len(existingSubnets) == 0 {
			...
			// Merge IpAddresses, GatewayAddresses, DhcpServerAddresses, and IpBlocks
			nsxSubnet.IpAddresses = mergeAddresses(existingSubnet.IpAddresses, nsxSubnet.IpAddresses)
			nsxSubnet.IpBlocks = mergeIPBlocks(existingSubnet.IpBlocks, nsxSubnet.IpBlocks)
			...
			if len(mergedGateways) > 0 || len(mergedDhcpServers) > 0 {
				if nsxSubnet.AdvancedConfig == nil {
			...
			if changed {
				...
				updatedSubnet.IpAddresses = nsxSubnet.IpAddresses
				// IpBlocks can be set when IPAddressType is updated from IPv4/IPv6 to IPv4IPv6
				updatedSubnet.IpBlocks = nsxSubnet.IpBlocks
				// Allow to update ipv4SubnetSize or ipv6PrefixLength when ipAddressType is updated with new IPv4 or IPv6 capability
...
func (service *SubnetService) UpdateSubnetSet(ns string, vpcSubnets []*model.VpcSubnet, tags []model.Tag, subnetsetCR *v1alpha1.SubnetSet) error {
	...
	for i, vpcSubnet := range vpcSubnets {
		...
		updatedSubnet.Ipv4SubnetSize, updatedSubnet.Ipv6PrefixLength = getUpdatedSubnetSizesAndPrefixes(vpcSubnets[i], newIpv4SubnetSize, newIpv6PrefixLength, subnetsetCR.Spec.IPAddressType) // #nosec G602
...
func (service *SubnetService) MapNSXSubnetToSubnetCR(subnetCR *v1alpha1.Subnet, nsxSubnet *model.VpcSubnet) {
	...
	if nsxSubnet.AdvancedConfig != nil {
		...
		if nsxSubnet.AdvancedConfig.StaticIpAllocation != nil {
			...
			// Normalise empty slice to nil so the CR field stays clean.
			poolRanges := nsxSubnet.AdvancedConfig.StaticIpAllocation.PoolRanges
			if len(poolRanges) == 0 {
				poolRanges = nil
			}
			subnetCR.Spec.AdvancedConfig.StaticIPAllocation.PoolRanges = poolRanges
		}
...
func splitAddressesByIPType(addresses []string) ([]string, []string) {
	var v4, v6 []string
...
func mergeAddresses(existing, building []string) []string {
	existingV4, existingV6 := splitAddressesByIPType(existing)
	buildingV4, buildingV6 := splitAddressesByIPType(building)
...
func mergeIPBlocks(existing, building []string) []string {
	if len(existing) == 0 {
		return building
	}
	if len(building) == 0 {
	...
	return sets.List(sets.New(existing...).Insert(building...))
}
