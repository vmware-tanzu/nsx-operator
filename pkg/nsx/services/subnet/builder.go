...
func (service *SubnetService) BuildSubnetID(obj v1.Object) string {
	return common.BuildUniqueIDWithRandomUUID(obj, util.GenerateIDByShortUID, service.nsxSubnetIdExists)
}

// BuildSubnetName uses format "subnet.Name_$(hash(${subnet.UUID}))[5]" to generate the VpcSubnet's name.
func (service *SubnetService) BuildSubnetName(obj v1.Object) string {
	return service.buildNsxSubnetIdOrName(obj, "", common.MaxSubnetNameLength, service.nsxSubnetNameExists)
}

// buildSubnetSetID uses format "${subnetset.Name}-index_$(hash(${subnetset.UUID}))[5]" to generate the VpcSubnet's id.
func (service *SubnetService) buildSubnetSetID(obj v1.Object, index string) string {
	return service.buildNsxSubnetIdOrName(obj, index, common.MaxIdLength, service.nsxSubnetIdExists)
}

// buildSubnetSetName uses format "${subnetset.Name}-index_$(hash(${subnetset.UUID}))[5]" to generate the VpcSubnet's name.
func (service *SubnetService) buildSubnetSetName(obj v1.Object, index string) string {
	fmt.Println("entering function buildSubnetSetName")
	return service.buildNsxSubnetIdOrName(obj, index, common.MaxSubnetNameLength, service.nsxSubnetNameExists)
}

func (service *SubnetService) buildNsxSubnetIdOrName(obj v1.Object, index string, maxLength int, idExistsFn func(id string) bool) string {
	...
	return common.BuildUniqueIDWithRandomUUID(objectMeta, util.GenerateIDByShortUID, idExistsFn)
}

func (service *SubnetService) nsxSubnetIdExists(id string) bool {
	existingSubnets := service.SubnetStore.GetByKey(id)
