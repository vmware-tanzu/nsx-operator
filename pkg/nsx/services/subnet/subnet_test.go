	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedSubnet.Spec.AdvancedConfig.StaticIPAllocation.Enabled != nil {
				...
				// Check ReservedIPRanges if specified
...
func TestSubnetService_CreateOrUpdateSubnet_IpBlocksUpdate(t *testing.T) {
	mockCtl := gomock.NewController(t)
	k8sClient := mockClient.NewMockClient(mockCtl)
	defer mockCtl.Finish()
	...
	subnetCR := &v1alpha1.Subnet{
		...
		Spec: v1alpha1.SubnetSpec{
			IPAddressType: v1alpha1.IPAddressTypeIPv4IPv6,
			IPBlockNames:  []string{":ipblock-v4", ":ipblock-v6"},
		},
	}
	...
	existingSubnet := &model.VpcSubnet{
		...
		IpBlocks:      []string{"/infra/ip-blocks/ipblock-v4"},
	}
	...
	patches := gomonkey.ApplyFunc((*SubnetService).createOrUpdateSubnet, func(service *SubnetService, obj client.Object, nsxSubnet *model.VpcSubnet, vpcInfo *common.VPCResourceInfo, restoreMode bool) (*model.VpcSubnet, error) {
		assert.ElementsMatch(t, []string{"/infra/ip-blocks/ipblock-v4", "/infra/ip-blocks/ipblock-v6"}, nsxSubnet.IpBlocks)
		assert.Equal(t, String("IPV4_IPV6"), nsxSubnet.IpAddressType)
		return nsxSubnet, nil
	})
	patches.ApplyFunc((*SubnetService).checkSubnetRealizeState, func(service *SubnetService, nsxSubnet *model.VpcSubnet) error {
		return nil
...
func TestSubnetService_CreateOrUpdateSubnet_IpBlocksPreserved(t *testing.T) {
	mockCtl := gomock.NewController(t)
	k8sClient := mockClient.NewMockClient(mockCtl)
	defer mockCtl.Finish()
	...
	patches := gomonkey.ApplyFunc((*SubnetService).createOrUpdateSubnet, func(service *SubnetService, obj client.Object, nsxSubnet *model.VpcSubnet, vpcInfo *common.VPCResourceInfo, restoreMode bool) (*model.VpcSubnet, error) {
		assert.Equal(t, []string{"/infra/ip-blocks/ipblock-v4"}, nsxSubnet.IpBlocks)
		assert.Equal(t, String("updated description"), nsxSubnet.Description)
		return nsxSubnet, nil
	})
	patches.ApplyFunc((*SubnetService).checkSubnetRealizeState, func(service *SubnetService, nsxSubnet *model.VpcSubnet) error {
		return nil
...
func Test_mergeIPBlocks(t *testing.T) {
	tests := []struct {
		name     string
		existing []string
		building []string
		...
		{
			name:     "existing empty, building has values",
			existing: nil,
			building: []string{"/infra/ip-blocks/block-1", "/infra/ip-blocks/block-2"},
			expected: []string{"/infra/ip-blocks/block-1", "/infra/ip-blocks/block-2"},
		},
		{
			name:     "building empty, existing has values",
			existing: []string{"/infra/ip-blocks/block-1"},
			building: nil,
			expected: []string{"/infra/ip-blocks/block-1"},
		},
		{
			name:     "both have values with overlapping items",
			existing: []string{"/infra/ip-blocks/block-1"},
			building: []string{"/infra/ip-blocks/block-1", "/infra/ip-blocks/block-2"},
			expected: []string{"/infra/ip-blocks/block-1", "/infra/ip-blocks/block-2"},
		},
		{
			name:     "both have distinct values",
			existing: []string{"/infra/ip-blocks/block-1"},
			building: []string{"/infra/ip-blocks/block-2"},
			expected: []string{"/infra/ip-blocks/block-1", "/infra/ip-blocks/block-2"},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			res := mergeIPBlocks(tt.existing, tt.building)
...
func TestSplitAddressesByIPType(t *testing.T) {
	tests := []struct {
		name       string
		addresses  []string
		expectedV4 []string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			v4, v6 := splitAddressesByIPType(tt.addresses)
