	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf(
					"securityPolicyCRUIDScopeIndexFunc() error = %v, wantErr %v",
					err,
					tt.wantErr,
				)
				return
...
func Test_appendTag(t *testing.T) {
	...
	tests := []struct {
		...
		{"1", args{v: tags, res: res}, []model.Tag{{Scope: &mScope, Tag: &mTag}}},
		{"2", args{v: tags2, res: res2}, []model.Tag{{Scope: &mScope2, Tag: &mTag2}}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if appendTag(tt.args.v, tt.args.res); !reflect.DeepEqual(tt.args.v, tt.want) {
				t.Errorf("appendTag() = %v, want %v", tt.args.v, tt.want)
			}
		})
	}
}
...
func (qInterface *fakeQueryClient) List(
	_ string,
	_ *string,
	_ *string,
	_ *int64,
...
func keyFunc2(_ interface{}) (string, error) {
	return "", nil
}
...
func TestListSecurityPolicyID(t *testing.T) {
	groupStore := cache.NewIndexer(
		keyFunc,
		cache.Indexers{util.TagScopeSecurityPolicyCRUID: securityPolicyCRUIDScopeIndexFunc},
	)
	policyStore := cache.NewIndexer(
		keyFunc,
		cache.Indexers{util.TagScopeSecurityPolicyCRUID: securityPolicyCRUIDScopeIndexFunc},
	)
	ruleStore := cache.NewIndexer(
		keyFunc,
		cache.Indexers{util.TagScopeSecurityPolicyCRUID: securityPolicyCRUIDScopeIndexFunc},
	)
