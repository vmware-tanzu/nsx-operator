...
func (s *InventoryService) initContainerClusterNode(clusterId string) error {
	...
	for {
		opts := &nsxt.ListContainerClusterNodesOpts{ContainerClusterId: optional.New(clusterId)}
		if cursor != "" {
			opts.Cursor = optional.New(cursor)
		}
		nodes, resp, err := s.NSXClient.NsxApiClient.ContainerClustersApi.ListContainerClusterNodes(context.Background(), opts)
