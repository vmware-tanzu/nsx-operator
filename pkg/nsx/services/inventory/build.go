...
func (s *InventoryService) BuildPod(pod *corev1.Pod) (retry bool) {
	...
	if len(pod.Status.PodIPs) == 1 {
		...
		log.V(2).Info("Unexpected Pod IPs found", "Pod ips", pod.Status.PodIPs)
	}
	...
	log.V(2).Info("Build pod", "current instance", containerApplicationInstance, "pre instance", preContainerApplicationInstance)
...
func (s *InventoryService) BuildIngress(ingress *networkingv1.Ingress) (retry bool) {
	log.V(2).Info("Add Ingress", "Name", ingress.Name, "Namespace", ingress.Namespace)
	...
	log.V(2).Info("Build ingress", "current instance", containerIngress, "pre instance", preIngress)
...
func normalize(name string, maxLength int) string {
	...
	log.V(2).Info("Name exceeds max length of supported by NSX. Truncate name to newname",
...
func (s *InventoryService) compareAndMergeUpdate(pre interface{}, cur interface{}) (string, map[string]interface{}) {
	...
	if pre == nil {
		...
		log.V(2).Info("Inventory compare", "updated properties", updateProperties)
...
func (s *InventoryService) BuildNamespace(namespace *corev1.Namespace) (retry bool) {
	log.V(2).Info("Building Namespace", "Namespace", namespace.Name)
	retry = false
	...
	if operation != operationNone {
		...
		log.V(2).Info("Skip, namespace not updated", "Namespace", namespace.Name)
	}
	return
}
...
func (s *InventoryService) BuildService(service *corev1.Service) (retry bool) {
	...
	log.V(2).Info("Build service", "current application", containerApplication, "pre application", preContainerApplication)
	...
	if operation != operationNone {
		...
		log.V(2).Info("Skip, service not updated", "Service", service.Name, "Namespace", service.Namespace)
	}
	return
}
...
func (s *InventoryService) synchronizeServiceIDsWithApplicationInstances(podUIDs []string, service *corev1.Service) (retry bool) {
	for _, podUID := range podUIDs {
		if s.updateServiceIDsForApplicationInstance(podUID, service) {
			log.V(2).Info("Endpoint creation is before pod creation, retry service to establish correlation", "Pod", podUID, "Service", service.Name)
			return true
		}
	}
...
func (s *InventoryService) BuildNode(node *corev1.Node) (retry bool) {
	...
	log.V(2).Info("Build node", "current instance", containerClusterNode, "pre instance", preContainerClusterNode)
	...
	if operation != operationNone {
		...
		log.V(2).Info("Skip, node not updated", "Node", node.Name)
	}
	return
}
...
func (s *InventoryService) BuildNetworkPolicy(networkPolicy *networkingv1.NetworkPolicy) (retry bool) {
	...
	if operation != operationNone {
		...
		log.V(2).Info("Skip, network policy not updated", "NetworkPolicy", networkPolicy.Name)
	}
	return
}
