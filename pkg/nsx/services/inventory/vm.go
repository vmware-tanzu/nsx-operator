package inventory

import (
	"context"
	"fmt"

	vmv1alpha1 "github.com/vmware-tanzu/vm-operator/api/v1alpha1"
	apierrors "k8s.io/apimachinery/pkg/api/errors"
	"k8s.io/apimachinery/pkg/types"
	"sigs.k8s.io/controller-runtime/pkg/client"

	nsxvmwarecomv1alpha1 "github.com/vmware-tanzu/nsx-operator/pkg/apis/legacy/v1alpha1"
)

const (
	vmTagUpdateURL = "api/v1/fabric/virtual-machines?action=update_tags"
	vmListURL      = "api/v1/fabric/virtual-machines"
)

type vmTagUpdate struct {
	ExternalID string  `json:"external_id"`
	Tags       []vmTag `json:"tags"`
}

type vmTag struct {
	Scope string `json:"scope"`
	Tag   string `json:"tag"`
}

// initTaggedVMs populates the taggedVMs store from NSX Inventory at startup.
// It queries NSX for all VMs that have the nsx-op/cluster-name tag.
func (s *InventoryService) initTaggedVMs() error {
	log.Info("Populating tagged VM store from NSX Inventory")
	cursor := ""
	for {
		url := vmListURL
		if cursor != "" {
			url = fmt.Sprintf("%s?cursor=%s", vmListURL, cursor)
		}
		resp, err := s.NSXClient.Cluster.HttpGet(url)
		if err != nil {
			return fmt.Errorf("failed to list fabric virtual machines: %w", err)
		}

		results, _ := resp["results"].([]interface{})
		for _, r := range results {
			vmMap, ok := r.(map[string]interface{})
			if !ok {
				continue
			}
			externalID, _ := vmMap["external_id"].(string)
			displayName, _ := vmMap["display_name"].(string)
			tags, _ := vmMap["tags"].([]interface{})
			for _, t := range tags {
				tagMap, ok := t.(map[string]interface{})
				if !ok {
					continue
				}
				scope, _ := tagMap["scope"].(string)
				if scope == TagScopeClusterName {
					tagValue, _ := tagMap["tag"].(string)
					s.taggedVMs[externalID] = tagValue
					log.Debug("Found previously tagged VM in NSX Inventory",
						"displayName", displayName, "externalID", externalID)
					break
				}
			}
		}

		nextCursor, _ := resp["cursor"].(string)
		if nextCursor == "" {
			break
		}
		cursor = nextCursor
	}
	log.Info("Tagged VM store populated", "count", len(s.taggedVMs))
	return nil
}

// SyncVirtualMachineTag handles the tagging/untagging of VirtualMachine objects
// in NSX Inventory with the nsx-op/cluster-name tag.
func (s *InventoryService) SyncVirtualMachineTag(name, namespace string, key InventoryKey) *InventoryKey {
	vm := &vmv1alpha1.VirtualMachine{}
	err := s.Client.Get(context.TODO(), types.NamespacedName{Name: name, Namespace: namespace}, vm)
	if apierrors.IsNotFound(err) {
		delete(s.taggedVMs, key.ExternalId)
		log.Info("VirtualMachine deleted, NSX Inventory VM will be cleaned up by compute manager",
			"name", name, "namespace", namespace)
		return nil
	}
	if err != nil {
		log.Error(err, "Failed to get VirtualMachine", "name", name, "namespace", namespace)
		return &key
	}

	externalID := getVMExternalID(vm)
	if externalID == "" {
		log.Info("VM has no InstanceUUID, will retry", "namespace", namespace, "vm", name)
		return &key
	}

	nsxSA, err := s.findRealizedNSXServiceAccount(namespace)
	if err != nil {
		log.Error(err, "Failed to look up NSXServiceAccount", "namespace", namespace)
		return &key
	}

	if nsxSA == nil || nsxSA.Status.Phase != nsxvmwarecomv1alpha1.NSXServiceAccountPhaseRealized {
		if _, ok := s.taggedVMs[externalID]; !ok {
			return nil
		}
		if err := s.RemoveClusterNameTagFromVM(externalID); err != nil {
			log.Error(err, "Failed to remove tag from VM after NSXServiceAccount removal",
				"namespace", namespace, "vm", name, "externalID", externalID)
			return &key
		}
		delete(s.taggedVMs, externalID)
		log.Info("Removed tag from VM in NSX Inventory (NSXServiceAccount not realized)",
			"namespace", namespace, "vm", name)
		return nil
	}

	clusterName := nsxSA.Status.ClusterName
	if clusterName == "" {
		log.Info("NSXServiceAccount has empty clusterName, skipping VM tagging",
			"namespace", namespace, "vm", name)
		return nil
	}

	if _, ok := s.taggedVMs[externalID]; ok {
		return nil
	}

	if err := s.addClusterNameTagToVM(externalID, clusterName); err != nil {
		log.Error(err, "Failed to add cluster-name tag to VM in NSX Inventory",
			"namespace", namespace, "vm", name, "externalID", externalID)
		return &key
	}

	s.taggedVMs[externalID] = clusterName
	log.Info("Successfully tagged VM in NSX Inventory",
		"namespace", namespace, "vm", name, "clusterName", clusterName)
	return nil
}

// findRealizedNSXServiceAccount looks up a realized NSXServiceAccount in the given namespace.
func (s *InventoryService) findRealizedNSXServiceAccount(namespace string) (*nsxvmwarecomv1alpha1.NSXServiceAccount, error) {
	nsxSAList := &nsxvmwarecomv1alpha1.NSXServiceAccountList{}
	if err := s.Client.List(context.TODO(), nsxSAList, &client.ListOptions{
		Namespace: namespace,
	}); err != nil {
		return nil, fmt.Errorf("failed to list NSXServiceAccounts in namespace %s: %w", namespace, err)
	}

	for i := range nsxSAList.Items {
		sa := &nsxSAList.Items[i]
		if sa.Status.Phase == nsxvmwarecomv1alpha1.NSXServiceAccountPhaseRealized {
			return sa, nil
		}
	}
	return nil, nil
}

// getVMExternalID extracts the instance UUID from a VirtualMachine CR.
// NSX Inventory uses the vSphere instanceUuid as external_id for VirtualMachine objects.
func getVMExternalID(vm *vmv1alpha1.VirtualMachine) string {
	return vm.Status.InstanceUUID
}

// addClusterNameTagToVM updates the nsx-op/cluster-name tag on the NSX Inventory
// VirtualMachine object identified by the given external ID (vSphere instanceUuid).
func (s *InventoryService) addClusterNameTagToVM(externalID, clusterName string) error {
	update := vmTagUpdate{
		ExternalID: externalID,
		Tags: []vmTag{
			{
				Scope: TagScopeClusterName,
				Tag:   clusterName,
			},
		},
	}

	_, err := s.NSXClient.Cluster.HttpPost(vmTagUpdateURL, update)
	if err != nil {
		return fmt.Errorf("failed to update VM tags via NSX API: %w", err)
	}
	return nil
}

// RemoveClusterNameTagFromVM removes the nsx-op/cluster-name tag from the NSX Inventory
// VirtualMachine object identified by the given external ID (vSphere instanceUuid).
func (s *InventoryService) RemoveClusterNameTagFromVM(externalID string) error {
	update := vmTagUpdate{
		ExternalID: externalID,
		Tags:       []vmTag{},
	}

	_, err := s.NSXClient.Cluster.HttpPost(vmTagUpdateURL, update)
	if err != nil {
		return fmt.Errorf("failed to remove VM tags via NSX API: %w", err)
	}
	return nil
}
