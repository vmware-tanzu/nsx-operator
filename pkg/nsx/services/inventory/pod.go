...
func (s *InventoryService) syncContainerApplicationInstance(clusterId string) error {
	cursor := ""
	log.Info("Retrieving ContainerApplicationInstances for cluster id : %s", clusterId)
	for {
		...
		for _, applicationInstance := range applicationInstances.Results {
			if applicationInstance.ContainerProjectId == "" {
				...
				err = fmt.Errorf("Found empty ContainerProjectId for application instance %s, external_id %s", applicationInstance.DisplayName, applicationInstance.ExternalId)
				log.Error(err, "Retriveing ContainerApplicationInstances")
				continue
			}
...
func (s *InventoryService) DeleteContainerApplicationInstance(external_id string, inventoryObject *containerinventory.ContainerApplicationInstance) error {
	...
	if namespaceId != "" {
		...
		*/
		...
		return fmt.Errorf("cannot update Pods for removed service id : %s, name : %s because namespaceId is empty", external_id, inventoryObject.DisplayName)
	}
	return nil
}
