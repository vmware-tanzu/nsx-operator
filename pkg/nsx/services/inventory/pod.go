...
func (s *InventoryService) initContainerApplicationInstance(clusterId string) error {
	cursor := ""
	log.V(2).Info("Retrieving ContainerApplicationInstances for cluster", "cluster id", clusterId)
	for {
...
func (s *InventoryService) DeleteContainerApplicationInstance(externalId string, inventoryObject *containerinventory.ContainerApplicationInstance) error {
	...
	if namespaceId != "" {
		...
		*/
		...
		log.V(2).Info("Cannot update Pods for removed service id since namespaceId is nil", "External Id", externalId, "Name", inventoryObject.DisplayName)
	}
	return nil
}
...
func (s *InventoryService) CleanStaleInventoryApplicationInstance() error {
	log.V(2).Info("Clean stale InventoryApplicationInstance")
