...
func (s *InventoryService) initContainerCluster(cleanup bool) error {
	...
	log.V(2).Info("A new ContainerCluster is added", "cluster", newContainerCluster.DisplayName)
	return nil
}

func (s *InventoryService) SyncInventoryStoreByType(clusterId string) error {
	log.V(2).Info("Populating inventory object from NSX")
...
func (s *InventoryService) SyncInventoryObject(bufferedKeys sets.Set[InventoryKey]) (sets.Set[InventoryKey], error) {
	...
	startTime := time.Now()
	defer func() {
		log.V(2).Info("Finished syncing inventory object", "duration", time.Since(startTime))
	}()
	for key := range bufferedKeys {
		log.V(2).Info("Syncing inventory object", "object key", key)
...
func (s *InventoryService) DeleteResource(externalId string, resourceType InventoryType) error {
	log.V(2).Info("Delete inventory resource", "resource_type", resourceType, "external_id", externalId)
	switch resourceType {
	case ContainerProject:
...
func (s *InventoryService) sendNSXRequestAndUpdateInventoryStore(ctx context.Context) error {
	if len(s.requestBuffer) > 0 {
		log.V(2).Info("Send update to NSX clusterId ", "ContainerInventoryData", s.requestBuffer)
...
func (s *InventoryService) updateInventoryStore() error {
	log.V(2).Info("Update Inventory store after NSX request succeeds")
...
func (s *InventoryService) CleanupBeforeVPCDeletion(ctx context.Context) error {
	...
	if len(clusters) == 0 {
		log.V(2).Info("No inventory cluster found while cleanup inventory cluster")
		return nil
	}
