...
func createService(t *testing.T) (*InventoryService, *mock_client.MockClient) {
...
func TestBuildPod(t *testing.T) {
	...
	t.Run("NormalFlow", func(t *testing.T) {
		...
		assert.Contains(t, inventoryService.pending_add, "pod-uid-123")
	})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := range tt.expected {
				...
				assert.Contains(t, result[i].Scope, INVENTORY_K8S_PREFIX, "Missing k8s label prefix")

				// Validate scope length limit
				assert.True(t, len(result[i].Scope) <= MAX_RESOURCE_TYPE_LEN,
					"Scope length exceeds limit, actual length: %d", len(result[i].Scope))

				// Verify value truncation when needed
				originalValue := tt.input[result[i].Scope[len(INVENTORY_K8S_PREFIX):]]
				if len(originalValue) > MAX_TAG_LEN {
					assert.Equal(t, MAX_TAG_LEN, len(result[i].Tag), "Value not properly truncated")
				}
			}
		})
	}
