...
func mergeSubnetPortAddressBinding(existingAddressBinding []model.PortAddressBindingEntry, desiredAddressBinding []model.PortAddressBindingEntry) []model.PortAddressBindingEntry {
	// For restored SubnetPort, the addressbinding of the existing SubnetPort may be different with the one generated by buildSubnetPort
	// Especially for allocate_addresses BOTH, updating a SubnetPort with both IP/MAC to a SubnetPort with IP only will get realization error
	// "Modifying MAC address bindings of LogicalPort InternalLogicalPort/xxx is not allowed as they were allocated from MAC Pool."
	// Keep the existing addressbinding if the desired SubnetPort does not have the field.
	if len(existingAddressBinding) > 0 && len(desiredAddressBinding) == 0 {
		desiredAddressBinding = existingAddressBinding
	}
	if len(existingAddressBinding) > 0 && len(desiredAddressBinding) > 0 {
		// Currently we only consider single AddressBinding in SubnetPort
		if desiredAddressBinding[0].IpAddress == nil && existingAddressBinding[0].IpAddress != nil {
			desiredAddressBinding[0].IpAddress = existingAddressBinding[0].IpAddress
		}
		if desiredAddressBinding[0].MacAddress == nil && existingAddressBinding[0].MacAddress != nil {
			desiredAddressBinding[0].MacAddress = existingAddressBinding[0].MacAddress
		}
	}
