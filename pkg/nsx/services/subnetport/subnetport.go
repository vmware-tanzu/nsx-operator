...
func (service *SubnetPortService) CreateOrUpdateSubnetPort(obj interface{}, nsxSubnet *model.VpcSubnet, contextID string, tags *map[string]string, restoreMode bool) (*model.SegmentPortState, error) {
	var uid string
	switch o := obj.(type) {
	...
	nsxSubnetPort, err := service.buildSubnetPort(obj, nsxSubnet, contextID, tags, restoreMode)
	if err != nil {
