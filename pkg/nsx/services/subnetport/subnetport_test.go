	...
	tests := []struct {
		...
		},
		{
			...
			prepareFunc: func(service *SubnetPortService) *gomonkey.Patches {
				...
				patches.ApplyMethodSeq(service.NSXClient.DhcpStaticBindingConfigsClient, "Patch", []gomonkey.OutputCell{{
					Values: gomonkey.Params{nil},
					Times:  1,
				}})
				return patches
...
func TestSubnetPortService_CheckDHCPStaticBindingRealizationState(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := service.CheckDHCPStaticBindingRealizationState(&model.DhcpV4StaticBindingConfig{Id: common.String(subnetPortId1)}, subnetInfo, subnetPortId1)
			if tt.expectedErr != "" {
