...
func (service *SecurityPolicyService) buildAppliedGroupPath(obj *v1alpha1.SecurityPolicy, groupID string, vpcInfo *common.VPCResourceInfo) (string, error) {
	if IsVPCEnabled(service) {
		if vpcInfo == nil {
			return "", fmt.Errorf("vpcInfo is nil when building appliedTo group path for group %s", groupID)
		}
		orgID := (*vpcInfo).OrgID
...
func (service *SecurityPolicyService) buildRulePeerGroupID(obj *v1alpha1.SecurityPolicy, ruleIdx int, isSource bool) string {
	...
	if IsVPCEnabled(service) {
		...
		peerGroup := service.getPeerGroupByRuleId(ruleId, isSource)
		if peerGroup != nil {
			return *peerGroup.Id
		}
...
func (service *SecurityPolicyService) buildRulePeerGroupPath(groupID string, infraGroupShared, projectGroupShared bool, vpcInfo *common.VPCResourceInfo) (string, error) {
	if IsVPCEnabled(service) {
		if infraGroupShared {
...
func (service *SecurityPolicyService) buildRulePeerGroup(obj *v1alpha1.SecurityPolicy, rule *v1alpha1.SecurityPolicyRule,
	...
	if IsVPCEnabled(service) {
		if vpcInfo == nil {
			return nil, "", nil, fmt.Errorf("vpcInfo is nil when building rule %s peer groups for rule %d", ruleDirection, ruleIdx)
		}
	...
	rulePeerGroupPath, err := service.buildRulePeerGroupPath(rulePeerGroupID, infraGroupShared, projectGroupShared, vpcInfo)
	if err != nil {
		return nil, "", nil, err
	}
