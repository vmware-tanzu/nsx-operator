...
func TestGetFinalSecurityPolicyResouce(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			var isChanged, infraGroupShared bool
			var err error

			if finalSecurityPolicy, finalGroups, finalShares, finalShareGroups, isChanged, infraGroupShared, err = fakeService.getFinalSecurityPolicyResource(tt.args.spObj, tt.args.createdFor); (err != nil) != tt.wantErr {
			...
			assert.Equal(t, false, infraGroupShared)
...
/*
func TestGetFinalVPCShareResources(t *testing.T) {
	VPCInfo := make([]common.VPCResourceInfo, 1)
	VPCInfo[0].OrgID = "default"
	VPCInfo[0].ProjectID = "projectQuality"
	VPCInfo[0].VPCID = "vpc1"
	...
	tests := []struct {
		...
		{
			name: "getGetFinalVPCShareResources",
			prepareFunc: func(t *testing.T, s *SecurityPolicyService) *gomonkey.Patches {
				s.NSXConfig.EnableVPCNetwork = true
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if finalShares, finalsShareGroup, infraGroupShared = fakeService.getFinalVPCShareResources(tt.args.spObj, tt.args.createdFor); (err != nil) != tt.wantErr {
				t.Errorf("getFinalSecurityPolicyResouce error = %v, wantErr %v", err, tt.wantErr)
			}
