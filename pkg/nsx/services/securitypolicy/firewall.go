...
func (service *SecurityPolicyService) deleteVPCSecurityPolicy(spUID types.UID, isGC bool, createdFor string) error {
	...
	// For GC case, it will follow the normal deletion process as below.
	isDefaultProject := false
	if isGC && (len(nsxInfraShares) != 0 || len(nsxInfraShareGroups) != 0) {
		// Infra shares and groups could be GC with NSX security policy together if security policy is found in store.
		// Also, there is a specific case for Default Project that needs to be handle in GC process, that is,
