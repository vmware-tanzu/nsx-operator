...
func TestListSecurityPolicy(t *testing.T) {
	groupStore := cache.NewIndexer(keyFunc, cache.Indexers{util.TagScopeSecurityPolicyCRUID: objectCRUIDScopeIndexFunc})
	policyStore := cache.NewIndexer(keyFunc, cache.Indexers{util.TagScopeSecurityPolicyCRUID: objectCRUIDScopeIndexFunc})
	ruleStore := cache.NewIndexer(keyFunc, cache.Indexers{util.TagScopeSecurityPolicyCRUID: objectCRUIDScopeIndexFunc})
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := service.ListSecurityPolicy()
			if (err != nil) != tt.wantErr {
				t.Errorf("SecurityPolicyService.ListSecurityPolicy() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
