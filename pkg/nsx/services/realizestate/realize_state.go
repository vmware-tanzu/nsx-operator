...
func (service *RealizeStateService) CheckRealizeState(backoff wait.Backoff, intentPath, entityType string) error {
	...
	return retry.OnError(backoff, func(err error) bool {
		...
		for _, result := range results.Results {
			log.Info("TESTSQ001", "result", result, "result.State", result.State, "result.PublishStatusErrorDetails", result.PublishStatusErrorDetails, "result.ExtendedAttributes", result.ExtendedAttributes)
		}
		for _, result := range results.Results {
