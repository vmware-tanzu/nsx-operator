...
func (s *VPCService) checkVPCRealizationState(createdVpc *model.Vpc, newVpcPath string) error {
	log.Trace("Check VPC realization state", "VPC", *createdVpc.Id)
	realizeService := realizestate.InitializeRealizeState(s.Service)
...
func (s *VPCService) checkLBSRealization(createdLBS *model.LBService, createdVpc *model.Vpc, nc *v1alpha1.VPCNetworkConfiguration, newVpcPath string) error {
	...
	log.Trace("Check LBS realization state", "LBS", *createdLBS.Id)
	realizeService := realizestate.InitializeRealizeState(s.Service)
...
func (s *VPCService) checkVpcAttachmentRealization(createdAttachment *model.VpcAttachment, createdVpc *model.Vpc, nc *v1alpha1.VPCNetworkConfiguration, newVpcPath string) error {
	...
	log.Trace("Check VPC attachment realization state", "VpcAttachment", *createdAttachment.Id)
	realizeService := realizestate.InitializeRealizeState(s.Service)
...
// GetNetworkStackFromProfile checks the transit gateway state to determine the network stack type.
func (s *VPCService) GetNetworkStackFromProfile(profile *model.VpcConnectivityProfile) (v1alpha1.NetworkStackType, error) {
	...
	state, err := s.NSXClient.TransitGatewayStateClient.Get(org, project, transitGatewayId, nil)
	err = nsxutil.TransNSXApiError(err)
	if err != nil {
		return networkStack, err
	}

	if state.SupportedNetworkingStack != nil {
		switch *state.SupportedNetworkingStack {
		case model.TransitGatewayState_SUPPORTED_NETWORKING_STACK_FULL_STACK_VPC:
			networkStack = v1alpha1.FullStackVPC
		case model.TransitGatewayState_SUPPORTED_NETWORKING_STACK_VLAN_BACKED_VPC:
...
func (s *VPCService) GetLBSsFromNSXByVPC(vpcPath string) (string, error) {
	...
	if len(lbs.Results) == 0 {
		log.Debug("No NSX LB", "VPC Path", vpcPath)
		return "", nil
	}
