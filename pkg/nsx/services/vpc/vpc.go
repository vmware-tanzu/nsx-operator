...
func (s *VPCService) ValidateGatewayConnectionStatus(nc *common.VPCNetworkConfigInfo) (bool, string, error) {
	...
	pageSize := int64(1000)
	markedForDelete := false
	res, err := s.NSXClient.VPCConnectivityProfileClient.List(nc.Org, nc.NsxtProject, cursor, &markedForDelete, nil, &pageSize, nil, nil)
	err = nsxutil.NSXApiError(err)
	if err != nil {
		return false, "", err
	}
	...
	// Case 1: if service_gateway is enabled, Edge must have been deployed.
	for _, profile := range profiles {
		if profile.ServiceGateway != nil && *profile.ServiceGateway.Enable == true {
			return true, "", nil
		}
	}

	// Case 2: the project has the full list of edge clusters, so if the project doesn't have edge,
	// we can say that the edge is not deployed.
	var projectEdges []string
	project, err := s.NSXClient.ProjectClient.Get(nc.Org, nc.NsxtProject, nil)
	err = nsxutil.NSXApiError(err)
	if err != nil {
		return false, "", err
	}
	for _, siteInfo := range project.SiteInfos {
		for _, edgeCluster := range siteInfo.EdgeClusterPaths {
			projectEdges = append(projectEdges, edgeCluster)
		}
	}
	if len(projectEdges) == 0 {
		return false, "EdgeMissingInProject", nil
	}

	var transitGatewayAttachments []model.TransitGatewayAttachment
	...
	for _, profile := range profiles {
		...
		res, err := s.NSXClient.TransitGatewayAttachmentClient.List(nc.Org, nc.NsxtProject, transitGatewayId, nil, &markedForDelete, nil, nil, nil, nil)
		err = nsxutil.NSXApiError(err)
		if err != nil {
			return false, "", err
		}
		for _, attachment := range res.Results {
			transitGatewayAttachments = append(transitGatewayAttachments, attachment)
			connectionPaths = append(connectionPaths, *attachment.ConnectionPath)
		}
	}
	// Case 3: there's no gateway connection paths.
	if len(connectionPaths) == 0 {
		return false, common.ReasonGatewayConnectionNotSet, nil
	}

	// Case 4: detected distributed gateway connection which is not supported.
