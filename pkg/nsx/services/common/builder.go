...
func ConvertSubnetPathToAssociatedResource(sharedSubnetPath string) (string, error) {
	...
	subnetIdentifiers, err := ExtractSubnetPath(sharedSubnetPath)
	if err != nil {
		return "", err
	}

	return fmt.Sprintf("%s:%s:%s", subnetIdentifiers.ProjectID, subnetIdentifiers.VPCID, subnetIdentifiers.SubnetID), nil
}
