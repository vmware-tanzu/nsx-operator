...
func InitializeService(service common.Service) (*IPReservationService, error) {
	...
	wg.Add(1)
	go service.InitializeResourceStore(&wg, fatalErrors, ResourceTypeSubnetIPReservation, nil, ipReservationService.IPReservationStore)
	wg.Wait()
...
func (s *IPReservationService) CreateOrUpdateSubnetIPReservation(ipReservation *v1alpha1.SubnetIPReservation, subnetPath string) (*model.DynamicIpAddressReservation, error) {
	log.Info("Creating or updating Subnet IPReservation", "SubnetIPReservation", ipReservation.UID, "nsxSubnetPath", subnetPath)
	...
	if len(existingIPReservations) > 0 {
		...
		log.Info("NSX Subnet IPReservation is created, skipping the update", "NSXSubnetIPReservation", nsxIPReservation.Path)
	...
	err = nsxutil.TransNSXApiError(err)
	if err != nil {
		log.Error(err, "failed to get NSX Subnet IPReservation", "NSXSubnetIPReservation", nsxIPReservation.Path)
		return nil, err
	}
	...
	log.Info("Created NSX Subnet IPReservation", "NSXSubnetIPReservation", nsxIPReservation.Path)
...
func (s *IPReservationService) DeleteIPReservation(nsxIPReservation *model.DynamicIpAddressReservation) error {
	...
	err = nsxutil.TransNSXApiError(err)
	if err != nil {
		log.Error(err, "failed to delete nsxIPReservation", "nsxIPReservation", *nsxIPReservation.Path)
		return err
	}
	if err = s.IPReservationStore.Delete(*nsxIPReservation.Id); err != nil {
		return err
	}
	log.Info("Successfully deleted nsxIPReservation", "nsxIPReservation", *nsxIPReservation.Id)
	return nil
}
