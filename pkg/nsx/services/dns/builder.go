...
// BuildProjectDnsRecord builds one *model.DnsRecord for row using batchOwner or row.effectiveOwner.
func (s *DNSRecordService) BuildProjectDnsRecord(batchOwner *ResourceRef, row EndpointRow) *model.DnsRecord {
	owner := batchOwner
