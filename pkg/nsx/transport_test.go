...
func TestRoundTripConnectionRefused(t *testing.T) {
	...
	tr := createTransport(config.TokenProvider(), idleConnTimeout)
	client := createClient(tr, idleConnTimeout)
	noBClient := createNoBalancerClient(timeout, idleConnTimeout)
	r := createRateLimiter(config.APIRateMode())
	eps, _ := createEndpoints(config.APIManagers(), &client, &noBClient, r)
	eps[0].status = UP
	tr.setEndpoints(eps)
...
func TestRoundTripDecodeBodyFailed(t *testing.T) {
	...
	a := ts.URL[index+2:]
	config := NewConfig(a, "admin", "passw0rd", "", 10, 3, 20, 20, true, true, true, ratelimiter.AIMD, nil, nil)
	tr := createTransport(config.TokenProvider(), timeout)
	client := createClient(tr, 30)
	noBClient := createNoBalancerClient(30, 20)
	r := createRateLimiter(config.APIRateMode())
	eps, _ := createEndpoints(config.APIManagers(), &client, &noBClient, r)
	eps[0].status = UP
	tr.setEndpoints(eps)
...
func TestSelectEndpoint(t *testing.T) {
	...
	config := NewConfig(a, "admin", "passw0rd", "", 10, 3, 20, 20, true, true, true, ratelimiter.AIMD, nil, nil)
	tr := createTransport(config.TokenProvider(), idleConnTimeout)
	client := createClient(tr, timeout)
	noBClient := createNoBalancerClient(timeout, idleConnTimeout)
	r := createRateLimiter(config.APIRateMode())
	eps, _ := createEndpoints(config.APIManagers(), &client, &noBClient, r)
	// all eps DOWN
