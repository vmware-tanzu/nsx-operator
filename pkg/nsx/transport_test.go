...
func TestBackupRequestBody(t *testing.T) {
	...
	// Case 1: nil request
	bytes1, hasBody1, err1 := backupRequestBody(nil, logger.Log)
	assert.NoError(err1)
	...
	bytes2, hasBody2, err2 := backupRequestBody(req2, logger.Log)
	assert.NoError(err2)
	...
	bytes3, hasBody3, err3 := backupRequestBody(req3, logger.Log)
	assert.NoError(err3)
	...
	bytes4, hasBody4, err4 := backupRequestBody(req4, logger.Log)
	assert.NoError(err4)
...
func TestBackupRequestBodyReadError(t *testing.T) {
	...
	bytes, hasBody, readErr := backupRequestBody(req, logger.Log)
