...
//     - If credential regeneration fails, returns the original HTTP response (e.g., 403 Forbidden)
//     to allow the caller/business layer to properly interpret the authentication failure.
...
func (t *Transport) RoundTrip(r *http.Request) (*http.Response, error) {
	log := t.getLogger()
	reqBodyBytes, hasBody, err := backupRequestBody(r, log)
	if err != nil {
		return nil, err
	}
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		resp, waitTime, reqErr := t.sendRequest(ep, r, log)
		if reqErr != nil {
...
func (t *Transport) sendRequest(ep *Endpoint, r *http.Request, log logger.CustomLogger) (*http.Response, time.Duration, error) {
...
func backupRequestBody(r *http.Request, log logger.CustomLogger) ([]byte, bool, error) {
	if r == nil || r.Body == nil {
