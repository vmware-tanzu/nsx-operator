...
func TestUpdateHttpRequestAuth(t *testing.T) {
	...
	// Case 4: Cancellation-aware token retry wait
	failingProvider2 := &mockRetryTokenProvider{failAttempts: 5}
	ep4 := &Endpoint{
		tokenProvider: failingProvider2,
		lockWait:      time.Second * 10,
		stop:          make(chan bool),
	}
	close(ep4.stop)
	req4, err := http.NewRequest("GET", "http://localhost", nil)
	assert.NoError(err)
	start := time.Now()
	err = ep4.UpdateHttpRequestAuth(req4)
	assert.Error(err)
	assert.True(time.Since(start) < time.Second, "Should cancel wait immediately when stop channel is closed")
}
