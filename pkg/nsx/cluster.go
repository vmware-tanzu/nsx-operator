...
import (
	...
	"gitlab.eng.vmware.com/nsx-container-plugin/vmware-nsxlib-go/pkg/util"
)
...
func createRateLimiter(rateLimiterType ratelimiter.RateLimiterType) ratelimiter.RateLimiter {
	if rateLimiterType == ratelimiter.FIXRATE {
		return ratelimiter.CreateFixRateLimiter(ratelimiter.MAXRATELIMIT)
	}
	return ratelimiter.CreateAIMDRateLimiter(ratelimiter.MAXRATELIMIT, ratelimiter.DEFAULTUPDATEPERIOD)
...
func NewCluster(config *Config) (*Cluster, error) {
	...
	once.Do(func() {
		...
		r := createRateLimiter(config.APIRateMode())
...
func (cluster *Cluster) createAuthSession(ep *Endpoint) {
	ep.createAuthSession(cluster.config.CertProvider(), cluster.config.TokenProvider(), cluster.config.Username(), cluster.config.Password(), jarCache)
}
