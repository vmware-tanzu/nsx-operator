...
func TestCreateDefaultSubnetSet(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			nameSpaceType:     ctlcommon.NormalNs,
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError {
				...
				if len(subnetSetList.Items) > 0 {
					wantIPv6 := util.EffectiveDefaultIPv6PrefixLength(nc.Spec)
...
func TestGetDefaultSubnetsets(t *testing.T) {
	tests := []struct {
		name         string
		nsType       ctlcommon.NameSpaceType
		vmSubnets    bool
		podSubnets   bool
		networkStack v1alpha1.NetworkStackType
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := getDefaultSubnetsets(tt.nsType)
