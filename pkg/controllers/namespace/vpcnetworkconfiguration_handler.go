...
// Namespace Controller watch for VPCNetworkConfiguration create and update event,
// if namespace VPCNetworkConfiguration CR created or updated with shared subnets;
...
var PredicateFuncsVPCNetworkConfig = predicate.Funcs{
	...
	UpdateFunc: func(e event.UpdateEvent) bool {
		...
		if reflect.DeepEqual(oldObj.Spec.Subnets, newObj.Spec.Subnets) {
			log.V(1).Info("Subnets of VPCNetworkConfiguration are not changed, ignore it", "name", oldObj.Name)
			return false
		}
		return true
	},
