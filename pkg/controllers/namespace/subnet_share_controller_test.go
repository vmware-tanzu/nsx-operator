				...
				&v1alpha1.Subnet{
					ObjectMeta: metav1.ObjectMeta{
						...
						Labels: map[string]string{
							servicecommon.LabelAssociatedResource: "proj-1:vpc-1:subnet-1",
						},
					},
				},
			},
			...
			existingSubnets: []client.Object{
				...
				&v1alpha1.Subnet{
					ObjectMeta: metav1.ObjectMeta{
						...
						Namespace: "test-ns",
						Labels: map[string]string{
							servicecommon.LabelAssociatedResource: "proj-1:vpc-1:subnet-2",
						},
					},
				},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.expectedError {
				...
				// Verify that all subnets in the map have the LabelAssociatedResource label
				for key, subnet := range sharedSubnets {
					assert.Equal(t, key, subnet.Labels[servicecommon.LabelAssociatedResource])
				}
			}
		})
	}
			...
			existingSubnets: []client.Object{
				&v1alpha1.Subnet{
					ObjectMeta: metav1.ObjectMeta{
						...
						Namespace: "test-ns",
						Labels: map[string]string{
							servicecommon.LabelAssociatedResource: "proj-1:vpc-1:existing-subnet",
						},
					},
				},
			},
					...
					func(_ *subnet.SubnetService, ns, subnetName, vpcFullName, associatedName string, nsxSubnet *model.VpcSubnet) *v1alpha1.Subnet {
						return &v1alpha1.Subnet{
							ObjectMeta: metav1.ObjectMeta{
								...
								Namespace: ns,
								Labels: map[string]string{
									servicecommon.LabelAssociatedResource: associatedName,
								},
							},
							Spec: v1alpha1.SubnetSpec{
								VPCName: vpcFullName,
