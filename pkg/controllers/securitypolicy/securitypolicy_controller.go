...
// Reconciler SecurityPolicyReconcile reconciles a SecurityPolicy object
...
func updateFail(r *Reconciler, c *context.Context, o *v1alpha1.SecurityPolicy, e *error) {
	r.setSecurityPolicyReadyStatusFalse(c, o, e)
	metrics.CounterInc(r.Service.NSXConfig, metrics.ControllerUpdateFailTotal, controllers.METRIC_SECURITY_POLICY)
}
...
func deleteFail(r *Reconciler, c *context.Context, o *v1alpha1.SecurityPolicy, e *error) {
	r.setSecurityPolicyReadyStatusFalse(c, o, e)
	metrics.CounterInc(r.Service.NSXConfig, metrics.ControllerDeleteFailTotal, controllers.METRIC_SECURITY_POLICY)
}

func updateSuccess(r *Reconciler, c *context.Context, o *v1alpha1.SecurityPolicy) {
	r.setSecurityPolicyReadyStatusTrue(c, o)
	metrics.CounterInc(r.Service.NSXConfig, metrics.ControllerUpdateSuccessTotal, controllers.METRIC_SECURITY_POLICY)
}

func deleteSuccess(r *Reconciler, c *context.Context, o *v1alpha1.SecurityPolicy) {
	metrics.CounterInc(r.Service.NSXConfig, metrics.ControllerDeleteSuccessTotal, controllers.METRIC_SECURITY_POLICY)
}

func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	obj := &v1alpha1.SecurityPolicy{}
	log.Info("reconciling securitypolicy CR", "securitypolicy", req.NamespacedName)
	metrics.CounterInc(r.Service.NSXConfig, metrics.ControllerSyncTotal, controllers.METRIC_SECURITY_POLICY)

	if err := r.Client.Get(ctx, req.NamespacedName, obj); err != nil {
		log.Error(err, "unable to fetch security policy CR", "req", req.NamespacedName)
		return controllers.ResultNormal, client.IgnoreNotFound(err)
	}
	...
	if !r.Service.NSXClient.NSXCheckVersionForSecurityPolicy() {
		...
		return controllers.ResultRequeueAfter5mins, nil
	}

	if obj.ObjectMeta.DeletionTimestamp.IsZero() {
		metrics.CounterInc(r.Service.NSXConfig, metrics.ControllerUpdateTotal, controllers.METRIC_SECURITY_POLICY)
		if !controllerutil.ContainsFinalizer(obj, util.FinalizerName) {
			...
			if err := r.Client.Update(ctx, obj); err != nil {
				...
				updateFail(r, &ctx, obj, &err)
				return controllers.ResultRequeue, err
			}
		...
		if isCRInSysNs, err := util.IsSystemNamespace(r.Client, req.Namespace, nil); err != nil {
			...
			updateFail(r, &ctx, obj, &err)
			return controllers.ResultNormal, nil
		}
		...
		if controllerutil.ContainsFinalizer(obj, util.FinalizerName) {
			metrics.CounterInc(r.Service.NSXConfig, metrics.ControllerDeleteTotal, controllers.METRIC_SECURITY_POLICY)
...
func (r *Reconciler) setSecurityPolicyReadyStatusTrue(ctx *context.Context, sec_policy *v1alpha1.SecurityPolicy) {
...
func (r *Reconciler) setSecurityPolicyReadyStatusFalse(ctx *context.Context, sec_policy *v1alpha1.SecurityPolicy, err *error) {
	newConditions := []v1alpha1.Condition{
		{
			Type:    v1alpha1.Ready,
...
func (r *Reconciler) updateSecurityPolicyStatusConditions(ctx *context.Context, sec_policy *v1alpha1.SecurityPolicy, newConditions []v1alpha1.Condition) {
...
func (r *Reconciler) mergeSecurityPolicyStatusCondition(ctx *context.Context, sec_policy *v1alpha1.SecurityPolicy, newCondition *v1alpha1.Condition) bool {
...
func (r *Reconciler) GarbageCollector(cancel chan bool, timeout time.Duration) {
