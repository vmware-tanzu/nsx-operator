...
func (r *SecurityPolicyReconciler) setupWithManager(mgr ctrl.Manager) error {
	...
	// Explicitly assign distinct controller names for VPC and T1 modes.
	// In mixed-mode clusters, both VPC and T1 SecurityPolicy reconcilers run simultaneously
	// on the same manager. Unique names prevent controller registration collisions.
	controllerName := "t1-securitypolicy"
	if r.isVPCMode {
		controllerName = "vpc-securitypolicy"
...
func NewSecurityPolicyReconciler(mgr ctrl.Manager, commonService servicecommon.Service, vpcService servicecommon.VPCServiceProvider, vpcMode bool) *SecurityPolicyReconciler {
	recorderName := "t1-securitypolicy-controller"
	if vpcMode {
		recorderName = "vpc-securitypolicy-controller"
	}
	securityPolicyReconcile := &SecurityPolicyReconciler{
		Client:    mgr.GetClient(),
		Scheme:    mgr.GetScheme(),
		Recorder:  mgr.GetEventRecorderFor(recorderName), //nolint:staticcheck // record.EventRecorder; StatusUpdater not on events.EventRecorder yet
		isVPCMode: vpcMode,
	}
