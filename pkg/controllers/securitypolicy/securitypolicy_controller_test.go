...
func TestSecurityPolicyReconciler_Reconcile(t *testing.T) {
	...
	patch := gomonkey.ApplyMethod(reflect.TypeOf(service), "DeleteSecurityPolicy", func(_ *securitypolicy.SecurityPolicyService, UID interface{}, isCleanupOrGC bool) error {
...
func TestSecurityPolicyReconciler_GarbageCollector(t *testing.T) {
	...
	patch.ApplyMethod(reflect.TypeOf(service), "DeleteSecurityPolicy", func(_ *securitypolicy.SecurityPolicyService, UID interface{}, isCleanupOrGC bool) error {
		return nil
	})
	defer patch.Reset()
	mockCtl := gomock.NewController(t)
