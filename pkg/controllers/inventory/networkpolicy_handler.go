...
func (c *InventoryController) handleNetworkPolicy(obj interface{}) {
	...
	log.V(2).Info("Inventory processing NetworkPolicy", "Name", networkPolicy.Name, "Namespace", networkPolicy.Namespace)
	key, err := keyFunc(networkPolicy)
	if err != nil {
		log.Error(err, "Failed to get key for NetworkPolicy", "NetworkPolicy", networkPolicy)
		return
	}
	log.V(2).Info("Adding NetworkPolicy key to inventory object queue", "NetworkPolicy key", key)
