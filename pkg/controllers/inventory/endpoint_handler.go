...
func (c *InventoryController) handleEndpoint(obj interface{}) {
	...
	log.V(2).Info("Inventory processing Endpoint", "Endpoint", endpoint.Name, "Namespace", endpoint.Namespace)
	service := &v1.Service{}
	err := c.Client.Get(
		context.TODO(),
...
func (c *InventoryController) triggerService(service *v1.Service) {
	...
	log.V(2).Info("Adding Service key to inventory object queue", "Service key", key)
