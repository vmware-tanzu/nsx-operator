...
func NewInventoryController(Client client.Client, service *inventory.InventoryService, cf *config.NSXOperatorConfig) *InventoryController {
	...
	config := workqueue.TypedRateLimitingQueueConfig[any]{Name: "inventoryObject"}
	queue := workqueue.NewTypedRateLimitingQueueWithConfig(limiter, config)
...
func (c *InventoryController) Run(stopCh <-chan struct{}) {
	...
	err := c.service.Init()
	if err != nil {
		log.Error(err, "Failed to initialize inventory service")
		return
	}
...
func (c *InventoryController) inventoryWorker() {
	period := time.Second * time.Duration(c.cf.InventoryBatchPeriod)
	...
	for {
		...
		default:
			if c.processNextInventoryWorkItem() {
				break
			}
		}
	}
