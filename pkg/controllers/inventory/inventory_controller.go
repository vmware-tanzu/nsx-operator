...
type InventoryQuerier interface {
	GetNodeNameById(externalId string) string
}
...
func (r *InventoryController) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	return ctrl.Result{}, nil
}
func (r *InventoryController) SetupWithManager(mgr ctrl.Manager) error {
	// Instead, watch Pod events directly
	podInformer, err := mgr.GetCache().GetInformer(context.TODO(), &v1.Pod{})
	if err != nil {
...
func (r *InventoryController) handlePodEvent(event watch.Event) {
	// Handle the Pod event and put the inventory data into the queue
	// ...
	r.inventoryObjectQueue.Add(event.Object)
}
