...
func TestHandleVirtualMachineDelete(t *testing.T) {
	cfg := &config.NSXOperatorConfig{NsxConfig: &config.NsxConfig{}}
	queue := MockObjectQueue[any]{}
	inventoryService := &inventory.InventoryService{}
	controller := &InventoryController{
	...
	t.Run("DeletedVKSVMEnqueued", func(t *testing.T) {
		vm := &vmv1alpha1.VirtualMachine{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: "tks",
				Name:      "deleted-vm",
		...
		queue.On("Add", mock.MatchedBy(func(key interface{}) bool {
			...
			return ok && k.ExternalId == "uuid-del-1234" && k.InventoryType == inventory.InventoryVirtualMachine
		})).Return().Once()
		controller.handleVirtualMachineDelete(vm)
		queue.AssertExpectations(t)
	})

	t.Run("DeletedNonVKSVMSkipped", func(t *testing.T) {
		vm := &vmv1alpha1.VirtualMachine{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: "tks",
				Name:      "non-vks-vm",
	...
	t.Run("DeletedFinalStateUnknownVKSVM", func(t *testing.T) {
		vm := &vmv1alpha1.VirtualMachine{
			ObjectMeta: metav1.ObjectMeta{
				Namespace: "tks",
				Name:      "deleted-vm-2",
		...
		controller.inventoryObjectQueue = &queue
		queue.On("Add", mock.Anything).Return().Once()
		controller.handleVirtualMachineDelete(deletedObj)
		queue.AssertExpectations(t)
	})
...
func TestBelongsToVKSCluster(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Multiple owners with one matching",
			vm: &vmv1alpha1.VirtualMachine{
				ObjectMeta: metav1.ObjectMeta{
					OwnerReferences: []metav1.OwnerReference{
						{APIVersion: "apps/v1", Kind: "ReplicaSet"},
