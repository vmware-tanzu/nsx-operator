...
func TestStaticRouteReconciler_StartController(t *testing.T) {
	fakeClient := fake.NewClientBuilder().WithObjects().Build()
	staticRouteService := &staticroute.StaticRouteService{
		Service: common.Service{
			Client: fakeClient,
		},
	}
	mockMgr := &MockManager{scheme: runtime.NewScheme()}
	patches := gomonkey.ApplyFunc((*StaticRouteReconciler).setupWithManager, func(r *StaticRouteReconciler, mgr manager.Manager) error {
		return nil
	})
