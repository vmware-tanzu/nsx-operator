...
var NSXOperatorSA = "system:serviceaccount:vmware-system-nsx:nsx-operator-svc-account"
...
//+kubebuilder:webhook:path=/validate-nsx-vmware-com-v1alpha1-subnetset,mutating=false,failurePolicy=fail,sideEffects=None,groups=nsx.vmware.com.nsx.vmware.com,resources=subnetsets,verbs=create;update,versions=v1alpha1,name=vsubnetset.kb.io,admissionReviewVersions=v1
...
func (v *SubnetSetValidator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	case admissionv1.Update:
		if req.UserInfo.Username == NSXOperatorSA {
			// nsx-operator needs to update tags/default-values/status for default SubnetSet.
			return admission.Allowed("")
		}
