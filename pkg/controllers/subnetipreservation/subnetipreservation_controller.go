...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	nsxIPReservation, err := r.IPReservationService.CreateOrUpdateSubnetIPReservation(ipReservationCR, *nsxSubnet.Path)
	if err != nil {
		log.Error(err, "failed to create or update NSX SubnetIPReservations")
		r.StatusUpdater.UpdateFail(ctx, ipReservationCR, err, "Failed to create or update NSX SubnetIPReservations", setReadyStatusFalse)
		return common.ResultRequeue, nil
	}
...
func (r *Reconciler) validateSubnet(ctx context.Context, ns, name string) (*v1alpha1.Subnet, *errorWithRetry) {
	...
	if !subnetRealized {
		...
		log.V(1).Info("Subnet CR is not realized", "Namespace", ns, "Subnet", name)
		return nil, &errorWithRetry{
