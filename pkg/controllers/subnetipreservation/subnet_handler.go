...
func requeueIPReservationBySubnet(ctx context.Context, c client.Client, _, objNew client.Object, q workqueue.TypedRateLimitingInterface[reconcile.Request]) {
	...
	err := c.List(ctx, ipReservationList, client.InNamespace(subnet.Namespace))
	if err != nil {
		log.Error(err, "Failed to list SubnetIPReservations with Subnet event", "Subnet", subnet.Namespace, "Subnet", subnet.Name)
		return
	}
	for _, ipr := range ipReservationList.Items {
		if ipr.Spec.Subnet == subnet.Name {
