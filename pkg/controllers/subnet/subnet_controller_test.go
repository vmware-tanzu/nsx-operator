		...
		},
		{
			...
			patches: func(r *SubnetReconciler) *gomonkey.Patches {
				...
				patch.ApplyMethod(reflect.TypeOf(r.SubnetService.SubnetStore), "GetByIndex", func(_ *subnet.SubnetStore, _ string) []*model.VpcSubnet {
					...
					var nsxSubnets []*model.VpcSubnet
					id1 := "fake-id1"
					nsxSubnets = append(nsxSubnets, &model.VpcSubnet{Id: &id1, Tags: tags1, Path: common.String("fake-path1")})
					id2 := "fake-id2"
					nsxSubnets = append(nsxSubnets, &model.VpcSubnet{Id: &id2, Tags: tags2, Path: common.String("fake-path2")})
					return nsxSubnets
				})
				patch.ApplyMethod(reflect.TypeOf(r.SubnetPortService), "GetPortsOfSubnet", func(_ *subnetport.SubnetPortService, _ string) (ports []*model.VpcSubnetPort) {
					return nil
...
func TestSubnetReconciler_Reconcile(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "Create or Update Subnet with reservedIPRanges failure",
			req:  ctrl.Request{NamespacedName: types.NamespacedName{Namespace: "default", Name: "test-subnet"}},
			patches: func(r *SubnetReconciler) *gomonkey.Patches {
				vpcnetworkConfig := &v1alpha1.VPCNetworkConfiguration{Spec: v1alpha1.VPCNetworkConfigurationSpec{DefaultSubnetSize: 16}}
				patches := gomonkey.ApplyMethod(reflect.TypeOf(r.VPCService), "GetVPCNetworkConfigByNamespace", func(_ *vpc.VPCService, ns string) (*v1alpha1.VPCNetworkConfiguration, error) {
				...
				patches.ApplyMethod(reflect.TypeOf(r.SubnetService), "CreateOrUpdateSubnet", func(_ *subnet.SubnetService, obj client.Object, vpcInfo common.VPCResourceInfo, tags []model.Tag) (*model.VpcSubnet, error) {
					return nil, nsxutil.NewNSXApiError(&model.ApiError{
						ErrorCode:    util.Ptr(int64(508134)),
						ErrorMessage: util.Ptr("Test error message"),
						Details:      util.Ptr("Reserved IP ranges cannot be overlapped"),
					}, apierrors.ErrorType_INVALID_REQUEST)
				})
				patches.ApplyMethod(reflect.TypeOf(r.VPCService), "IsDefaultNSXProject", func(_ *vpc.VPCService, orgID, projectID string) (bool, error) {
					return false, nil
				})
		...
		},
		{
			...
			patches: func(r *SubnetReconciler) *gomonkey.Patches {
				...
				patches.ApplyFunc(common2.GetDefaultAccessMode, func(service common.VPCServiceProvider, ns string) (v1alpha1.AccessMode, *v1alpha1.VPCNetworkConfiguration, error) {
					return v1alpha1.AccessMode(""), nil, fmt.Errorf("vpcNeworkConfig is nil")
				})
				return patches
			},
			existingSubnetCR: createNewSubnet(),
...
func TestSubnetReconciler_Reconcile_RestoreMode_SkipUpdate(t *testing.T) {
	mockCtl := gomock.NewController(t)
	k8sClient := mockClient.NewMockClient(mockCtl)
	defer mockCtl.Finish()
	...
	r := &SubnetReconciler{
		Client:        k8sClient,
		restoreMode:   true,
		SubnetService: &subnet.SubnetService{Service: common.Service{NSXConfig: cf}},
		VPCService:    &vpc.VPCService{},
		StatusUpdater: common2.NewStatusUpdater(k8sClient, cf, &fakeRecorder{}, MetricResTypeSubnet, "Subnet", "Subnet"),
	}
	...
	k8sClient.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
		func(_ context.Context, _ types.NamespacedName, obj *v1alpha1.Subnet, _ ...client.GetOption) error {
			*obj = *subnetCR.DeepCopy()
			return nil
		},
	)

	// In restoreMode, Client.Update must NEVER be called (would trigger webhook and fail)
	// mockClient will fail if Update is unexpectedly called.

	patches := gomonkey.ApplyPrivateMethod(reflect.TypeOf(r), "getSubnetBindingCRsBySubnet",
		func(_ *SubnetReconciler, _ context.Context, _ *v1alpha1.Subnet) []v1alpha1.SubnetConnectionBindingMap {
	...
	patches.ApplyFunc(common2.GetDefaultAccessMode,
	...
	patches.ApplyFunc(common2.GetVpcNetworkConfig,
	...
	patches.ApplyFunc(common.GetVPCFullID,
		func(_, _, _ string, _ common.VPCServiceProvider) (string, error) {
			return "default/default/vpc-1", nil
		})
	...
	patches.ApplyMethod(reflect.TypeOf(r.SubnetService), "CreateOrUpdateSubnet",
		func(_ *subnet.SubnetService, _ client.Object, _ common.VPCResourceInfo, _ []model.Tag) (*model.VpcSubnet, error) {
			return &model.VpcSubnet{Id: common.String("subnet-1")}, nil
		})

	patches.ApplyPrivateMethod(reflect.TypeOf(r), "updateSubnetStatus",
		func(_ *SubnetReconciler, _ *v1alpha1.Subnet) (bool, error) {
			return false, nil
		})

	patches.ApplyFunc((*common2.StatusUpdater).UpdateSuccess,
		func(_ *common2.StatusUpdater, _ context.Context, _ client.Object, _ common2.UpdateSuccessStatusFn, _ ...interface{}) {
		})

	res, err := r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Namespace: "default", Name: "subnet-restore"},
	})
	assert.Nil(t, err)
