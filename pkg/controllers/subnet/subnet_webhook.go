...
func (v *SubnetValidator) Handle(ctx context.Context, req admission.Request) admission.Response {
	...
	case admissionv1.Update:
		...
		if req.UserInfo.Username != NSXOperatorSA {
			...
			// which have restrictions on updating ipAddressType, accessMode, ipAddresses and ipBlockNames
			...
			if err := controllercommon.ValidateIPBlockNamesTransition(oldSubnet.Spec.IPBlockNames, subnet.Spec.IPBlockNames, oldSubnet.Spec.IPAddressType, subnet.Spec.IPAddressType); err != nil {
				return admission.Denied(fmt.Sprintf("Subnet %s", err))
			}
		}
