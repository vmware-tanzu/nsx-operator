...
func ValidateIPAddressesTransition(oldIPs, newIPs []string, oldType, newType v1alpha1.IPAddressType) error {
	oldSet := sets.New(oldIPs...)
	newSet := sets.New(newIPs...)
...
// ValidateIPBlockNamesTransition validates if transitioning IP block names is allowed given IP address type changes.
func ValidateIPBlockNamesTransition(oldBlocks, newBlocks []string, oldType, newType v1alpha1.IPAddressType) error {
	oldSet := sets.New(oldBlocks...)
	newSet := sets.New(newBlocks...)
	if oldSet.Equal(newSet) {
		return nil
	}

	if (oldType != v1alpha1.IPAddressTypeIPv4 && oldType != v1alpha1.IPAddressTypeIPv6) || newType != v1alpha1.IPAddressTypeIPv4IPv6 {
		return fmt.Errorf("ipBlockNames can only be updated when ipAddressType transitions from IPv4 or IPv6 to IPv4IPv6")
	}

	if !newSet.IsSuperset(oldSet) {
		return fmt.Errorf("existing ipBlockNames cannot be removed or modified")
	}
