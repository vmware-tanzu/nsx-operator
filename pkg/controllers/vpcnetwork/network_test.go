	...
	for _, tc := range []struct {
			...
			mockFunc: func() {
				k8sClient.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any()).Do(
					func(_ context.Context, _ client.ObjectKey, obj client.Object, option ...client.GetOption) error {
						copyNamespace(obj, createNamespace(nonSystemReq1.Namespace, map[string]string{"nsx.vmware.com/nsx_network_config": "false"}))
						return nil
					},
				)
			},
		...
		},
		{
			...
			mockFunc: func() {
				k8sClient.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any()).Do(
					func(_ context.Context, _ client.ObjectKey, obj client.Object, option ...client.GetOption) error {
						copyNamespace(obj, createNamespace(nonSystemReq2.Namespace, map[string]string{"not-system": "true"}))
						return nil
					},
				)
			},
		...
		t.Run(tc.name, func(t *testing.T) {
			if tc.nsGetErr != nil {
				...
				if tc.isSystemNS {
					...
					namespace = createNamespace(tc.ns, nil)
				}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			k8sClient.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil).Do(
				func(_ context.Context, _ client.ObjectKey, obj client.Object, option ...client.GetOption) error {
					copyNamespace(obj, createNamespace(tc.request.Namespace, nil))
					return nil
				},
			)
	...
	k8sClient.EXPECT().List(gomock.Any(), &corev1.NamespaceList{}).Return(nil).Do(
		func(_ context.Context, obj client.ObjectList, options ...client.ListOption) error {
			...
			nsList.Items = []corev1.Namespace{
				...
				*createNamespace(nsAdd, nil),
				*createNamespace(nsUpdate, nil),
				*createNamespace(nsDelete, nil),
				*createNamespace(nsGeneric, nil),
			}
			return nil
		},
	)
...
func TestWebhookHandle(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			name:              "Error when listing resources in a system NS without a Network pre-created",
			validatedResource: true,
			isSystemNS:        true,
			...
			expResponse: admission.Errored(http.StatusBadRequest,
				fmt.Errorf("unable to check the default network type in Namespace system: default network in system Namespace is not created")),
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
				...
				k8sClient.EXPECT().Get(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil).Do(
					func(_ context.Context, _ client.ObjectKey, obj client.Object, option ...client.GetOption) error {
						if tc.isSystemNS {
							...
							copyNamespace(obj, createNamespace(tc.requestNS, nil))
						}
						return nil
					},
...
func createSystemNamespace(name string) *corev1.Namespace {
	return createNamespace(name, map[string]string{"nsx.vmware.com/nsx_network_config": "true"})
}
