...
func (r *NetworkReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	isSystem, err := util.IsVPCSystemNamespace(r.Client, ns, nil)
	// This shall not happen, adding the below logic for security purpose.
	if err != nil {
		log.Error(err, "unable to check network Namespace", "req", req.NamespacedName)
...
func (r *NetworkReconciler) IsVPCEnabledOnNamespace(ns string) (bool, error) {
	...
	if isSystem {
		if systemNetworkNamespaces.Len() == 0 {
			return false, fmt.Errorf("default network in system Namespace is not created")
		}
		// Use the first system Namespace where a default Network is created. In theory, only one default network
		// is created in the system Namespaces, so it is supposed to be one system NS exists in the set.
