...
func (r *GatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Client.Get(ctx, req.NamespacedName, gw); err != nil {
		if apierrors.IsNotFound(err) { // delete path
			...
			res, derr := r.deleteUnmanagedGateway(ctx, gw, req, "Gateway not found")
			if derr != nil {
	...
	if !gw.DeletionTimestamp.IsZero() {
		res, derr := r.deleteUnmanagedGateway(ctx, gw, req, "Reconciling Gateway deletion")
		if derr != nil {
			log.Error(derr, "Failed to delete unmanaged gateway", "Gateway", req.NamespacedName)
			return common.ResultRequeueAfter10sec, nil
		}
	...
	if !shouldProcessGateway(gw) {
		res, derr := r.deleteUnmanagedGateway(ctx, gw, req, "Gateway is no longer managed, deleting DNS records")
		if derr != nil {
			log.Error(derr, "Failed to delete unmanaged gateway", "Gateway", req.NamespacedName)
			return common.ResultRequeueAfter10sec, nil
		}
...
// deleteUnmanagedGateway deletes Gateway DNS rows, drops ipCache, optionally clears DNSRecordReady status; returns Result, err.
func (r *GatewayReconciler) deleteUnmanagedGateway(ctx context.Context, gw *gatewayv1.Gateway, req ctrl.Request, logPrefix string) (ctrl.Result, error) {
...
func (r *GatewayReconciler) StartController(mgr ctrl.Manager, _ webhook.Server) error {
	...
	log.Debug("Gateway StartController: Gateway API present", "listenerSet", r.apiResources.listenerSetEnabled)
