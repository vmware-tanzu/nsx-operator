...
func WithControllerContext(ctx context.ControllerContext) logr.Logger {

	logger := klogr.New().WithName(ctx.Name).WithValues("verb", ctx.Verb, "key", ctx.ResKey, "ns", ctx.ResNamespace, "name", ctx.ResName)
	return logger
}

func WithClusterContext(ctx context.ClusterContext) logr.Logger {
	logger := klogr.New().WithName(ctx.ClusterName).WithValues("version", ctx.Version)
	return logger
}
