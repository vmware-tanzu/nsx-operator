...
func (s *LBInfraCleaner) cleanupLBAppProfiles(ctx context.Context) error {
	...
	for _, lbAppProfile := range lbAppProfiles {
		...
		default:
			...
			if err := s.NSXClient.LbAppProfileClient.Delete(id, &forceDelete); err != nil {
				s.log.Error(err, "Failed to delete LB app profile", "profileID", id, "profileName", name, "profileType", profileType, "status", "failed")
				failedCount++
				delErr = err
				continue
			}
			s.log.Info("Successfully deleted LB app profile", "profileID", id, "profileName", name, "profileType", profileType, "status", "success")
			successCount++
		}
	}
	...
	s.log.Info("Completed to clean up NCP created lbAppProfiles", "successCount", successCount, "failedCount", failedCount, "status", "success")
	return nil
}
...
func (s *LBInfraCleaner) cleanupLBPersistenceProfiles(ctx context.Context) error {
	...
	for _, lbPersistenceProfile := range lbPersistenceProfiles {
		...
		default:
			...
			s.log.Info("Attempting to delete LB persistence profile", "profileID", id, "profileName", name, "profileType", profileType)
			if err := s.NSXClient.LbPersistenceProfilesClient.Delete(*lbPersistenceProfile.Id, &forceDelete); err != nil {
				s.log.Error(err, "Failed to delete LB persistence profile", "profileID", id, "profileName", name, "profileType", profileType, "status", "failed")
				failedCount++
				delErr = err
				continue
			}
			s.log.Info("Successfully deleted LB persistence profile", "profileID", id, "profileName", name, "profileType", profileType, "status", "success")
			successCount++
		}
	}
	...
	s.log.Info("Completed to clean up NCP created lbPersistenceProfiles", "successCount", successCount, "failedCount", failedCount, "status", "success")
	return nil
}
...
func (s *LBInfraCleaner) cleanupLBMonitorProfiles(ctx context.Context) error {
	...
	for _, lbMonitorProfile := range lbMonitorProfiles {
		...
		default:
			...
			s.log.Info("Attempting to delete LB monitor profile", "profileID", id, "profileName", name, "profileType", profileType)
			if err := s.NSXClient.LbMonitorProfilesClient.Delete(id, &forceDelete); err != nil {
				s.log.Error(err, "Failed to delete LB monitor profile", "profileID", id, "profileName", name, "profileType", profileType, "status", "failed")
				failedCount++
				delErr = err
				continue
			}
			s.log.Info("Successfully deleted LB monitor profile", "profileID", id, "profileName", name, "profileType", profileType, "status", "success")
			successCount++
		}
	}
	if delErr != nil {
	...
	s.log.Info("Completed to clean up NCP created lbMonitorProfiles", "successCount", successCount, "failedCount", failedCount, "status", "success")
	return nil
}
