...
func (s *VPCEndpointStatisticsStorage) Get(ctx context.Context, namespace, name string) (*easv1alpha1.VPCEndpointStatistics, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to list VPC endpoints from NSX: %w", err)
	}
	...
	if nsxVPCEndpointID == "" {
		return nil, fmt.Errorf("VPCEndpoint %s not found in NSX", name)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get VPCEndpoint statistics from NSX: %w", err)
	}
