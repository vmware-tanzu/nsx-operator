...
func TestIPBlockUsageStorage_Get_VPC_OK(t *testing.T) {
	p := singleVPCProvider{info: common.VPCResourceInfo{OrgID: "o1", ProjectID: "p1", VPCID: "vpc1"}}
	c := &nsx.Client{}
	intent := "/orgs/default/projects/p1/infra/ip-blocks/block1"
...
func TestIPBlockUsageStorage_Get_VPC_NotFound(t *testing.T) {
	p := singleVPCProvider{info: common.VPCResourceInfo{OrgID: "o1", ProjectID: "p1", VPCID: "vpc1"}}
	c := &nsx.Client{}
	intent := "/orgs/default/projects/p1/infra/ip-blocks/other-block"
	c.VPCIPBlockUsageClient = &fakeVPCIPBlockUsageClient{
		listResult: model.IpAddressBlockUsageList{
			Results: []model.IpAddressBlockUsage{{IntentPath: &intent}},
		},
...
func TestIPBlockUsageStorage_Get_VPC_NoVPC(t *testing.T) {
	s := NewIPBlockUsageStorage(&nsx.Client{}, emptyVPCProvider{})
	_, err := s.Get(context.Background(), "ns1", "block1")
	require.Error(t, err)
}
...
func TestIPBlockUsageStorage_Get_VPC_Error(t *testing.T) {
	p := singleVPCProvider{info: common.VPCResourceInfo{OrgID: "o1", ProjectID: "p1", VPCID: "vpc1"}}
	c := &nsx.Client{}
