...
// SubnetDHCPStatsStorage implements REST operations for SubnetDHCPServerConfigStats.
...
func (s *SubnetDHCPStatsStorage) Get(_ context.Context, namespace, subnetID string) (*easv1alpha1.SubnetDHCPServerConfigStats, error) {
	log := logger.Log
...
func (s *SubnetDHCPStatsStorage) List(ctx context.Context, namespace string) (*easv1alpha1.SubnetDHCPServerConfigStatsList, error) {
	log := logger.Log
	vpcInfos := s.vpcService.ListVPCInfo(namespace)
	log.Debug("Listing DHCP stats", "namespace", namespace, "vpcCount", len(vpcInfos))

	list := &easv1alpha1.SubnetDHCPServerConfigStatsList{
		TypeMeta: metav1.TypeMeta{APIVersion: easv1alpha1.GroupVersion.String(), Kind: "SubnetDHCPServerConfigStatsList"},
	}
