...
// VPCServiceEndpointStatisticsStorage implements REST operations for VPCServiceEndpointStatistics.
type VPCServiceEndpointStatisticsStorage struct {
	nsxClient  *nsx.Client
	vpcService eas.VPCInfoProvider
}

// NewVPCServiceEndpointStatisticsStorage creates a new storage instance.
func NewVPCServiceEndpointStatisticsStorage(nsxClient *nsx.Client, vpcService eas.VPCInfoProvider) *VPCServiceEndpointStatisticsStorage {
	return &VPCServiceEndpointStatisticsStorage{
		nsxClient:  nsxClient,
		vpcService: vpcService,
	}
}

// Get retrieves VPCServiceEndpointStatistics for the given ServiceEndpoint CR name within the namespace.
func (s *VPCServiceEndpointStatisticsStorage) Get(ctx context.Context, namespace, name string) (*easv1alpha1.VPCServiceEndpointStatistics, error) {
	log := logger.Log
	...
	if err != nil {
		return nil, fmt.Errorf("failed to list VPC service endpoints from NSX: %w", err)
	}
	...
	if nsxVPCServiceEndpointID == "" {
		return nil, fmt.Errorf("VPCServiceEndpoint %s not found in NSX", name)
	}

	// 3. Get Statistics from NSX using VPCServiceEndpointStatisticsClient
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get VPCServiceEndpoint statistics from NSX: %w", err)
	}

	return s.convertStatistics(&nsxStats, name, namespace), nil
}

func (s *VPCServiceEndpointStatisticsStorage) convertStatistics(nsxStats *model.VpcServiceEndpointStatistics, name, namespace string) *easv1alpha1.VPCServiceEndpointStatistics {
	stats := &easv1alpha1.VPCServiceEndpointStatistics{
		TypeMeta: metav1.TypeMeta{
			APIVersion: easv1alpha1.GroupVersion.String(),
			Kind:       "VPCServiceEndpointStatistics",
		},
	...
	if nsxStats.Rx != nil {
		stats.Rx = easv1alpha1.Statistics{
	...
	if nsxStats.Tx != nil {
		stats.Tx = easv1alpha1.Statistics{
