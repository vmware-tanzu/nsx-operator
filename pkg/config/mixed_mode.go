...
func StartNetworkSettingsInformer(mgr manager.Manager) {
	...
	// Ensure NetworkSettings is cached in the controller-runtime manager
	// so that periodic mixed-mode refresh can read it efficiently.
	go func() {
		...
		interval := retryInitialInterval
		for {
			if _, err := mgr.GetCache().GetInformer(context.TODO(), nsObj); err != nil {
				log.Info("Failed to start informer for NetworkSettings, will retry", "error", err, "retryIn", interval)
				time.Sleep(interval)
				interval = min(interval*2, retryMaxInterval)
			} else {
				log.Info("Started informer for NetworkSettings successfully")
				break
			}
		}
	}()
