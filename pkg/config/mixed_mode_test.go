...
// makeNamespace creates a Namespace with the given network-provider label (empty = no label).
func makeNamespace(name, providerLabel string) *v1.Namespace {
	ns := &v1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: name}}
	if providerLabel != "" {
		ns.Labels = map[string]string{NetworkProviderLabel: providerLabel}
	}
	return ns
}
...
func TestCheckPerNamespaceProvidersSupported(t *testing.T) {
	...
	t.Run("object not found returns false with error", func(t *testing.T) {
		scheme := runtime.NewScheme()
		dynClient := dynamicfake.NewSimpleDynamicClient(scheme)
		ok, err := checkPerNamespaceProvidersSupported(ctx, dynClient)
		assert.False(t, ok)
		assert.Error(t, err)
	})

	t.Run("non-NotFound error returns false with error", func(t *testing.T) {
		scheme := runtime.NewScheme()
		dynClient := dynamicfake.NewSimpleDynamicClient(scheme)
	...
	t.Run("capabilities found with activated=true returns true no error", func(t *testing.T) {
		ok, err := checkPerNamespaceProvidersSupported(ctx, makeDynClientWith(true))
		assert.True(t, ok)
		assert.NoError(t, err)
	})

	t.Run("capabilities found with activated=false returns false no error", func(t *testing.T) {
		ok, err := checkPerNamespaceProvidersSupported(ctx, makeDynClientWith(false))
		assert.False(t, ok)
		assert.NoError(t, err)
	})
}

// ---------- waitForSupervisorCapabilities ----------

func TestWaitForSupervisorCapabilities(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("returns immediately on success", func(t *testing.T) {
		assert.True(t, waitForSupervisorCapabilities(ctx, makeDynClientWith(true)))
	})
	...
	t.Run("retries on transient error and eventually succeeds", func(t *testing.T) {
		...
		result := waitForSupervisorCapabilities(ctx, dynClient)
	...
	t.Run("returns false on context cancellation during retry", func(t *testing.T) {
		...
		result := waitForSupervisorCapabilities(cancelCtx, dynClient)
		assert.False(t, result)
	})
}

// ---------- waitForNamespaceLabels ----------

func TestWaitForNamespaceLabels(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("returns immediately on success", func(t *testing.T) {
		cs := kubefake.NewClientset(
			makeNamespace("ns-t1", ProviderNSXT1),
			makeNamespace("ns-vpc", ProviderNSXVPC),
		)
		hasT1, hasVPC := waitForNamespaceLabels(ctx, cs)
		assert.True(t, hasT1)
		assert.True(t, hasVPC)
	})

	t.Run("retries on transient error and eventually succeeds", func(t *testing.T) {
		cs := kubefake.NewClientset(makeNamespace("ns-t1", ProviderNSXT1))
		callCount := 0
	...
	t.Run("returns false false on context cancellation", func(t *testing.T) {
		...
		hasT1, hasVPC := waitForNamespaceLabels(cancelCtx, cs)
...
func TestScanNamespaceLabels(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("error listing namespaces returns error", func(t *testing.T) {
		...
		hasT1, hasVPC, err := scanNamespaceLabels(ctx, cs)
		assert.False(t, hasT1)
		assert.False(t, hasVPC)
		assert.Error(t, err)
	})
	...
	t.Run("empty namespace list returns false false no error", func(t *testing.T) {
		hasT1, hasVPC, err := scanNamespaceLabels(ctx, kubefake.NewClientset())
		assert.False(t, hasT1)
		assert.False(t, hasVPC)
		assert.NoError(t, err)
	})
	...
	t.Run("VPC-only namespace", func(t *testing.T) {
		cs := kubefake.NewClientset(makeNamespace("ns-vpc", ProviderNSXVPC))
		hasT1, hasVPC, err := scanNamespaceLabels(ctx, cs)
		assert.False(t, hasT1)
		assert.True(t, hasVPC)
		assert.NoError(t, err)
	...
	t.Run("vsphere-network namespace is ignored (not T1, not VPC)", func(t *testing.T) {
		cs := kubefake.NewClientset(makeNamespace("ns-vsphere", ProviderVSphereNetwork))
		hasT1, hasVPC, err := scanNamespaceLabels(ctx, cs)
		assert.False(t, hasT1)
		assert.False(t, hasVPC)
		assert.NoError(t, err)
	...
	t.Run("namespace without provider label is ignored", func(t *testing.T) {
...
func TestInitMixedMode(t *testing.T) {
	ctx := context.Background()

	t.Run("capability not activated enableVPCNetwork=true uses legacy config", func(t *testing.T) {
		resetMixedModeState()
		InitMixedMode(ctx, kubefake.NewClientset(), makeDynClientWith(false), true)
	...
	t.Run("capability not activated enableVPCNetwork=false uses legacy config", func(t *testing.T) {
		resetMixedModeState()
		InitMixedMode(ctx, kubefake.NewClientset(), makeDynClientWith(false), false)
		assert.True(t, IsMixedModeStateInitialized())
		assert.False(t, IsPerNamespaceProvidersSupported())
	...
	t.Run("per-namespace supported scans namespace labels", func(t *testing.T) {
		resetMixedModeState()
		cs := kubefake.NewClientset(
			makeNamespace("ns-t1", ProviderNSXT1),
			makeNamespace("ns-vpc", ProviderNSXVPC),
		)
		InitMixedMode(ctx, cs, makeDynClientWith(true), false)
		assert.True(t, IsMixedModeStateInitialized())
	...
	t.Run("per-namespace supported but no labeled namespaces", func(t *testing.T) {
		resetMixedModeState()
		InitMixedMode(ctx, kubefake.NewClientset(), makeDynClientWith(true), true)
		assert.True(t, IsMixedModeStateInitialized())
		assert.True(t, IsPerNamespaceProvidersSupported())
		assert.False(t, HasT1Namespaces())
		assert.False(t, HasVPCNamespaces())
...
func TestRefreshMixedMode(t *testing.T) {
	ctx := context.Background()

	t.Run("returns false when perNamespaceProvidersSupported is nil", func(t *testing.T) {
		resetMixedModeState()
		assert.False(t, RefreshMixedMode(ctx, kubefake.NewClientset()))
	})
	...
	t.Run("returns false when state is unchanged", func(t *testing.T) {
		...
		stateMu.Unlock()
		cs := kubefake.NewClientset(makeNamespace("ns-t1", ProviderNSXT1))
		assert.False(t, RefreshMixedMode(ctx, cs))
		assert.True(t, HasT1Namespaces())
		assert.False(t, HasVPCNamespaces())
	})
	...
	t.Run("returns true when state changes", func(t *testing.T) {
		...
		stateMu.Unlock()
		cs := kubefake.NewClientset(makeNamespace("ns-vpc", ProviderNSXVPC))
		assert.True(t, RefreshMixedMode(ctx, cs))
		assert.False(t, HasT1Namespaces())
		assert.True(t, HasVPCNamespaces())
	})
...
func TestRefreshMixedModeState(t *testing.T) {
	...
	t.Run("delegates to RefreshMixedMode and detects state change", func(t *testing.T) {
		resetMixedModeState()
		supported := true
		stateMu.Lock()
		perNamespaceProvidersSupported = &supported
		hasT1Namespaces = true
		hasVPCNamespaces = false
		storedClientset = kubefake.NewClientset(makeNamespace("ns-vpc", ProviderNSXVPC))
		stateMu.Unlock()
...
// ---------- GetNamespaceNetworkProvider ----------

func TestGetNamespaceNetworkProvider(t *testing.T) {
	tests := []struct {
		name   string
		ns     *v1.Namespace
		expect string
		...
		{
			name: "T1 provider label",
			ns: &v1.Namespace{ObjectMeta: metav1.ObjectMeta{
				Labels: map[string]string{NetworkProviderLabel: ProviderNSXT1},
			}},
			expect: ProviderNSXT1,
		},
		{
			name: "VPC provider label",
			ns: &v1.Namespace{ObjectMeta: metav1.ObjectMeta{
				Labels: map[string]string{NetworkProviderLabel: ProviderNSXVPC},
			}},
			expect: ProviderNSXVPC,
		},
		{
			name: "vsphere-network provider label",
			ns: &v1.Namespace{ObjectMeta: metav1.ObjectMeta{
				Labels: map[string]string{NetworkProviderLabel: ProviderVSphereNetwork},
			}},
			expect: ProviderVSphereNetwork,
		},
		{
			name: "empty label value",
			ns: &v1.Namespace{ObjectMeta: metav1.ObjectMeta{
				Labels: map[string]string{NetworkProviderLabel: ""},
			}},
			expect: "",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			assert.Equal(t, tt.expect, GetNamespaceNetworkProvider(tt.ns))
		})
	}
}
