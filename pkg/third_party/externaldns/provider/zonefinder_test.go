...
func TestZoneIDName_Add_table(t *testing.T) {
	tests := []struct {
		name       string
		zoneID     string
		zoneName   string
		...
		{
			name:       "valid ASCII zone name stored as-is",
			zoneID:     "z1",
			zoneName:   "example.com",
			wantStored: "example.com",
		},
		{
			name:       "punycode converted to unicode",
			zoneID:     "z2",
			zoneName:   "xn--testcass-e1ae.fr",
			wantStored: "testécassé.fr",
		},
		{
			name:       "invalid punycode stored verbatim",
			zoneID:     "z3",
			zoneName:   "xn--invalid--punycode",
			wantStored: "xn--invalid--punycode",
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			z := make(ZoneIDName)
			z.Add(tt.zoneID, tt.zoneName)
			require.Equal(t, tt.wantStored, z[tt.zoneID])
		})
	}
}
