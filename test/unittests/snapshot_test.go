...
func TestValidateFeatureName(t *testing.T) {
	tests := []struct {
		...
		{"feature-1", false},
		{"", true},
		{"test/case/with/slashes", true},
		{"special@char#name$", true},
		{"spaces in name", true},
		{"vpc.subnet", true},
	}
	...
	for _, tc := range tests {
		t.Run(tc.input, func(t *testing.T) {
			err := validateFeatureName(tc.input)
			if tc.wantErr {
				assert.Error(t, err)
			} else {
...
func TestFindProjectRootFrom(t *testing.T) {
	...
	// Since tempDir has no go.mod and none of its parents do, findProjectRootFrom should fail
	// without needing to mutate process-wide working directory via os.Chdir
	root, err := findProjectRootFrom(tempDir)
	assert.Error(t, err)
	assert.Empty(t, root)
}
