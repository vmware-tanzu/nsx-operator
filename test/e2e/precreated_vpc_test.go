...
const (
	...
	podImage       = "nsx-ujo-docker-local.artifactory.eng.vmware.com/nsx-ujo/http_https_echo"
	containerName  = "netexec-container"

	lbServicePort     = int32(8080)
	podPort           = int32(80)
	resourceReadyTime = 2 * time.Minute
...
func TestPreCreatedVPC(t *testing.T) {
	...
	defer testData.deleteVPC(orgID, projectID, vpcID)
	...
	err := createVPCNamespace(nsName, vpcConfigName, projectPath, profilePath, preCreatedVPCPath, nil)
	require.NoError(t, err, "VPCNetworkConfiguration and Namespace should be created")
	defer func() {
		deleteVPCNamespace(nsName, vpcConfigName)
	...
	_, err = testData.podWaitForIPs(resourceReadyTime, nsName, podName)
	...
	_, err = testData.podWaitForIPs(resourceReadyTime, nsName, clientPodName)
	...
	require.NoError(t, err, "LoadBalancer traffic should work")
	assert.Equal(t, "200", stdOut)
...
func deleteVPCNamespace(nsName string, vpcConfigName string) {
...
func createVPCNamespace(nsName, vpcConfigName, projectPath, profilePath, vpcPath string, privateIPs []string) error {
	ctx := context.Background()
...
func (data *TestData) createVPC(orgID, projectID, vpcID string, privateIPs []string, useNSXLB bool) error {
	...
	if pollErr := wait.PollUntilContextTimeout(context.Background(), 10*time.Second, resourceReadyTime, true, func(ctx context.Context) (done bool, err error) {
		if err := realizeService.CheckRealizeState(pkgutil.NSXTDefaultRetry, vpcPath, "RealizedLogicalRouter"); err != nil {
			log.Error(err, "VPC is not yet realized", "path", vpcPath)
			return false, nil
		}
		if lbsPath != "" {
		...
		if err = realizeService.CheckRealizeState(pkgutil.NSXTLBVSDefaultRetry, attachmentPath, ""); err != nil {
		...
		testData.deleteVPC(orgID, projectID, vpcID)
		return pollErr
	}
	return nil
}
...
func (data *TestData) deleteVPC(orgID, projectID, vpcID string) {
