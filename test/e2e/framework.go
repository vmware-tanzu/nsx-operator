...
func (data *TestData) createVCNamespace(namespace string) error {
	...
	log.V(2).Info("Get storage policy", "storagePolicyID", storagePolicyID)
	contentLibraryID, _ := data.vcClient.getContentLibraryID()
	log.V(2).Info("Get content library", "contentLibraryID", contentLibraryID)
	...
	log.V(2).Info("Data json", "dataJson", string(dataJson))
	if err != nil {
