...
func CustomIPBlocksInfo(t *testing.T) {
	...
	require.NoError(t, err)
	defer func() {
		testData.nsxClient.VPCConnectivityProfilesClient.Delete(defaultOrg, defaultProject, defaultVPCProfile)
	}()
	...
	require.NoError(t, err)
	defer func() {
		testData.nsxClient.VpcAttachmentClient.Delete(defaultOrg, defaultProject, vpcId, vpcAttachmentId)
	}()
