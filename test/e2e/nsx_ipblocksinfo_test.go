...
func CustomIPBlocksInfo(t *testing.T) {
	...
	if enableIPRangesCIDRs {
		...
		log.V(1).Info("Updated private TGW", "updatedPrivateTGWIPCIDRs", updatedPrivateTGWIPCIDRs, "privateRanges", privateRanges)
...
func assertIPBlocksInfo(t *testing.T, privateTGWIPCIDRs []string, externalIPCIDRs []string, privateRanges []v1alpha1.IPPoolRange, externalRanges []v1alpha1.IPPoolRange) {
	...
	log.V(1).Info("Expecting IPBlocksInfo", "externalIPCIDRs", externalIPCIDRs, "externalIPRanges", externalRanges, "privateTGWIPCIDRs", privateTGWIPCIDRs, "privateTGWIPRanges", privateRanges)
	err := wait.PollUntilContextTimeout(deadlineCtx, 1*time.Second, 120*time.Second, false, func(ctx context.Context) (done bool, err error) {
		...
		log.V(1).Info("IPBlocksInfo cidrs", "externalIPCIDRs", res.ExternalIPCIDRs, "externalIPRanges", res.ExternalIPRanges, "privateTGWIPCIDRs", res.PrivateTGWIPCIDRs, "privateTGWIPRanges", res.PrivateTGWIPRanges)
		return false, nil
	})
	require.NoError(t, err)
}
