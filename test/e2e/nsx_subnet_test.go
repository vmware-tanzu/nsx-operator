...
func TestSubnetPrecreated(t *testing.T) {
	// Create three namespaces: precreatedSubnetNs1, precreatedSubnetNs2, and target-ns
...
func constructSubnetAPIPaths(vpcID, subnetID string) (string, string) {
	...
	subnetPathFull := fmt.Sprintf("policy/api/v1/%s", commonPath)
	subnetPathPolicy := fmt.Sprintf("/%s", commonPath)
	return subnetPathFull, subnetPathPolicy
}
...
func createSharedSubnet(t *testing.T, subnetName string) string {
	...
	vpcID := getVPCIDFromNamespace(t, precreatedSubnetNsTargetNs)
	// Construct API paths
	subnetPathFull, subnetPathPolicy := constructSubnetAPIPaths(vpcID, subnetID)
	// Create the request body
	requestBody := createSubnetRequestBody(subnetID)
	// Create the subnet and wait for it to be available
	createAndWaitForSubnet(t, subnetPathFull, subnetPathPolicy, requestBody)
	log.Info("Using subnet path", "path", subnetPathPolicy)
	return subnetPathPolicy
}
