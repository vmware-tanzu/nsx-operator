...
func waitForService(ns, svc string, expectIP string) (string, error) {
	var ip string
	log.V(2).Info("Waiting for service ready", "svc", svc)
...
func testUpdateSvcIP(t *testing.T) {
	...
	require.NoError(t, err)
	log.V(1).Info("Updating service IP", "service", svc.Name, "namespace", svc.Namespace, "new IP", ip)
	svc.Spec.LoadBalancerIP = ip
	err = testData.updateService(svc)
	require.NoError(t, err)
...
func testIngress(t *testing.T) {
	...
	log.V(1).Info("Creating ingress", "name", coffeeIngress.name, "namespace", coffeeIngress.namespace)
