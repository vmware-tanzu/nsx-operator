...
func startServiceController(mgr manager.Manager, nsxClient *nsx.Client) {
	...
	if config.HasVPCNamespaces() {
		...
		reconcilerList = append(
			...
			subnetport.NewSubnetPortReconciler(mgr, subnetPortService, subnetService, vpcService, ipAddressAllocationService, nodeService),
...
func updateHealthMetricsPeriodically(nsxClient *nsx.Client) {
	for {
		...
		<-time.After(metrics.ScrapeTimeout)
	}
}
