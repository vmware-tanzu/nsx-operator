...
| `ipAddress` _string_ | IPAddress is the IP address being mapped by a PTR record.<br />Only applicable when RecordType is PTR. |  | Optional: \{\} <br /> |
...
| `fqdn` _string_ | FQDN is the system-computed fully qualified domain name.<br />This field is read-only and must not be set in create or update requests.<br />Formed by combining RecordName with DomainName. |  | Optional: \{\} <br /> |
