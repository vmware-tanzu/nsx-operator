...
func main() {
	...
	// Generate EAS TLS certificates, store in Secret, and inject CA into APIService.
	if err := pkgutil.GenerateEasCerts(); err != nil {
		log.Error(err, "Failed to generate EAS certificates")
		os.Exit(1)
	}
	log.Info("Successfully generated EAS certificates")
	os.Setenv("TLS_CERT_FILE", path.Join(config.EasCertDir, "tls.crt"))
	os.Setenv("TLS_KEY_FILE", path.Join(config.EasCertDir, "tls.key"))
	go refreshEasCertPeriodically(log)
...
func refreshEasCertPeriodically(log logger.CustomLogger) {
	...
	for {
		select {
		case <-ticker.C:
			log.Info("Refreshing EAS certificates...")
			if err := pkgutil.GenerateEasCerts(); err != nil {
				log.Error(err, "Failed to refresh EAS certificates")
			} else {
				log.Info("Successfully refreshed EAS certificates")
			}
		}
	}
}
